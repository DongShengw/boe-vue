<template>
  <div v-wechat-title="$route.meta.title" class="home" style="padding: 20px">
    ...
    <div style="margin: 10px 0">
      ...
      <el-button type="primary" :disabled="isAll" @click="Delete"
        >批量删除</el-button
      >
    </div>
    ...
    >
      ...
      <el-table-column prop="noticeState" label="公告状态">
        <template #default="scope">
          ...
          <el-tag v-if="scope.row.noticeState == 3" type="success"
            >待发布</el-tag
          >
          <el-tag v-if="scope.row.noticeState == 2" type="warning"
            >播放中</el-tag
          >
          <el-tag v-if="scope.row.noticeState == 1" type="info"
            >发布失败</el-tag
          >
          <el-tag v-if="scope.row.noticeState == 0" type="danger"
            >已结束</el-tag
          >
        </template>
      </el-table-column>
